 ...
 * </p>
 * <br/>
 * If a <code>void _writeJson(Writer writer)</code> method is found on an
 * object, then that method will be called to output the JSON for that object. A
 * corresponding <code>void _readJson(Map map)</code> method is looked for and
 * called if it exists. These should rarely (if ever) be used. See the
 * TestJsonReaderWriter for an example of this usage.<br/>
 * <br/>
 * 
...
public class JsonWriter extends Writer {
	 ...
	 *             If an I/O error occurs
	 */
	public static String objectToJson(Object item, boolean prettyMode)
			throws IOException {
	...
	private void traceReferences(Object root) {
		...
		while (!stack.isEmpty()) {
			...
			if (isPrimitiveWrapper(clazz) || clazz.equals(String.class)
					|| clazz.equals(Class.class)) { // Can't reference anything
				continue;
			}

			if (clazz.isArray()) {
				Class compType = clazz.getComponentType();
				if (!compType.isPrimitive()) { // Speed up: do not trace
					// references of primitive array
					// types - they cannot reference
					// objects, other than
					// immutables.
	...
	private void traceFields(LinkedList<Object> stack, Object obj) {
		...
		for (Field field : fields.values()) {
			try {
				if (isPrimitiveWrapper(field.getType())) { // speed up:
					// primitives &
					// primitive
					// wrappers cannot
					// reference another
	...
	private boolean writeOptionalReference(Object obj) throws IOException {
		if (_objVisited.containsKey(obj)) { // Only write (define) an object
			...
			_out.write("{\"@ref\":");
			String id = getId(obj);
			if (id == null) { // TODO: remove this if once it is determined this
				// can never happen
				_out.write("0}");
				return true;
			}
	...
	private void writeImpl(Object obj, boolean showType) throws IOException {
		...
		if (obj.getClass().isArray()) {
			...
			if (_prettyMode) { // Optional [default] special handling for
				// Collection's and Maps to make output smaller,
				// easier to read.
	...
	private void writeIdAndType(Object col, boolean showType, boolean referenced)
			throws IOException {
		if (showType || referenced) {
			if (referenced) {
				String id = getId(col);
				if (id == null) { // TODO: Remove this check if we determine
	...
	/**
	 * Specialized output for certain classes to make them easy to read and
	 * create.
	 */
	private void writeSpecial(Object obj, boolean showType, String value)
			throws IOException {
			...
			for (int i = 0; i < len; i++) {
				...
				if (value == null) {
					...
					if (value instanceof Boolean || value instanceof Long
							|| value instanceof Double) { // These types can be
						// inferred - true |
						// false, long
						// integer number (-
						// or digits), or
	...
	private void writeObject(Object obj, boolean showType) throws IOException {
		...
		if (classInfo._writeMethod != null) { // Must show type when class has
			// custom _writeJson() method on
			// it.
		...
		if (classInfo._writeMethod == null) {
			for (Field field : classInfo.values()) {
				if (_prettyMode
						&& (field.getModifiers() & Modifier.TRANSIENT) != 0) { // Skip
					// transient
					// fields
					// when
					// in
			...
			try {
				...
				throw new IOException("Error invoking " + obj.getClass()
						+ "._jsonWrite()", e);
			}
		}
	 ...
	 *         methods if they exist. The results are cached internally for
	 *         performance when called again with same Class.
	 */
	static ClassMeta getDeepDeclaredFields(Class c) {
		...
		try {
			classInfo._writeMethod = c.getDeclaredMethod("_writeJson",
					new Class[] { Writer.class });
		} catch (Exception ignored) {
		}

		try {
			classInfo._readMethod = c.getDeclaredMethod("_readJson",
					new Class[] { Map.class });
		} catch (Exception ignored) {
		}
