 ...
 * <ul>
 * <li>
 * Call the static method: <code>JsonReader.toJava(String json)</code>. This
 * will return a typed Java object graph.</li>
 * <li>
 * Call the static method: <code>JsonReader.toMaps(String json)</code>. This
 ...
 * associated Map's values. In this representation: All integer numbers are
 * stored as java.lang.Long objects. All floating point numbers are stored as
 * java.lang.Double instances. Booleans are stored as java.lang.Boolean. All
 * other values are stored as Strings.</li>
 * <li>
 ...
 * class that was the root of the graph.</li>
 * <li>
 * Instantiate the JsonReader with an InputStream:
 * <code>JsonReader(InputStream in, true)</code> and then call
 * <code>readObject()</code>. The return value will be a Map of Maps.</li>
 * </ul>
 * <br/>
 * 
...
public class JsonReader extends Reader {
	...
	/**
	 * LinkedHashMap used to keep fields in same order as they are when
	 * reflecting them in Java. Instances of this class hold a Map-of-Map
	...
	public static Object jsonToJava(String json) throws IOException {
		ByteArrayInputStream ba = new ByteArrayInputStream(json
				.getBytes("UTF-8"));
	 ...
	 *             for stream errors or parsing errors.
	 */
	private Object convertMapsToObjects(JsonObject<String, Object> root)
			throws IOException {
		...
		while (!stack.isEmpty()) {
			...
			if (javaMate.getClass().isArray()) { // Handle assigning Map to
													// javaMate that is an []
				traverseArray(stack, jsonObj);
			} else if ((javaMate instanceof Collection)
					&& jsonObj.containsKey("@items")) {
				traverseCollection(stack, jsonObj);
			} else if (javaMate instanceof Map && jsonObj.containsKey("@keys")
					&& jsonObj.containsKey("@items")) {
				traverseMap(stack, jsonObj);
			} else {
				if (javaMate instanceof Map) {
					_maps.add((Map) javaMate); // List of maps to be rebuilt
												// (re-indexed)
				}
	 ...
	 *             for stream errors or parsing errors.
	 */
	private void traverseArray(LinkedList<JsonObject<String, Object>> stack,
		...
		if (byte.class.equals(compType)) { // Handle byte[] special for
											// performance boost.
		...
		for (int i = 0; i < len; i++) {
			...
			if (element == null) {
			...
			} else if (element instanceof JsonArray) { // Array of arrays
				if (char[].class.equals(compType)) { // Specially handle char[]
														// because we are
														// writing these
					...
					Array.set(array, i, createJavaObjectInstance(compType,
							jsonObject));
				...
				if (ref != null) { // Connect reference
					...
					if (refObject != null && refObject.target != null) { // Array
																			// element
																			// with
																			// @ref
																			// to
	...
	private void traverseCollection(
			LinkedList<JsonObject<String, Object>> stack, JsonObject jsonObj)
			throws IOException {
		JsonArray items = (JsonArray) jsonObj.get("@items");
		...
		while (i.hasNext()) {
			...
			if (element == null) {
				...
				throw new IOException("Encountered unknown type - "
	 ...
	 */
	private void traverseMap(LinkedList<JsonObject<String, Object>> stack,
		...
		if (keys == null || items == null) {
			if (keys != items) {
				throw new IOException(
						"Map written where one of @keys or @items is empty, jsonObject:\n"
								+ jsonObj);
			}
			return;
		}
		...
		if (javaKeys.size() != javaValues.size()) {
			throw new IOException(
					"Map written with @keys and @items entries of different sizes, jsonObject:\n"
							+ jsonObj);
		}
	...
	private void traverseFields(LinkedList<JsonObject<String, Object>> stack,
			JsonObject<String, Object> jsonObj) throws IOException {
		Object javaMate = jsonObj.target;

		JsonWriter.ClassMeta meta = JsonWriter.getDeepDeclaredFields(javaMate
				.getClass());
		...
		if (meta._readMethod != null) {
			try {
				...
				throw new IOException("Unabled to invoke "
						+ javaMate.getClass().getName() + "."
						+ meta._readMethod.getName(), e.getTargetException());
			} catch (IllegalAccessException e) {
				throw new IOException("Unabled to invoke "
						+ javaMate.getClass().getName() + "."
						+ meta._readMethod.getName(), e);
			}
		} else {
			Iterator<Map.Entry<String, Object>> i = jsonObj.entrySet()
					.iterator();
	 ...
	 *             for stream errors or parsing errors.
	 */
	private void assignField(LinkedList<JsonObject<String, Object>> stack,
		...
		try {
			...
			if (rhs == null) {
			...
			} else if (rhs instanceof JsonArray) { // LHS of assignment is an []
													// field OR right handside
													// is an array and LHS is
													// Object
				JsonArray elements = (JsonArray) rhs;
				JsonObject<String, Object> jsonArray = new JsonObject<String, Object>();
				if (char[].class.equals(fieldType)) { // Specially handle char[]
														// because we are
														// writing these
					...
					if (elements.size() == 0) {
						...
						field.set(target, ((String) elements.get(0))
								.toCharArray());
					}
				} else {
				...
				if (ref != null) { // Correct field references
					...
					field
							.set(target, createJavaObjectInstance(fieldType,
									jObj));
					if (!isPrimitive(jObj.target.getClass())) {
						stack.addFirst((JsonObject) rhs);
					}
				}
			} else if (isPrimitive(fieldType)) { // field.set() will convert
			...
			throw new IOException("IllegalAccessException setting field '"
	 ...
	 * <p/>
	 * The '@type' is not usually specified that much in the JSON input stream,
	 * as in many cases it can be inferred from a field reference or array
	 * component type.
	 * 
	 * @param clazz
	 ...
	 *             for stream errors or parsing errors.
	 */
	private Object createJavaObjectInstance(Class clazz, JsonObject jsonObj)
			throws IOException {
		...
		if (type != null) { // @type is explicitly set, use that as it always
			...
			if (clazz.isArray()
					|| (items != null && clazz.equals(Object.class))) {
				if (items == null) {
					throw new IOException(
							"'@items' not specified for a JSON array, no way to determine length");
				}

				mate = Array.newInstance(clazz.isArray() ? clazz
						.getComponentType() : Object.class, items.size());
			} else { // Definitely not an [] type.
				if (isPrimitive(clazz)) { // If it is a primitive wrapper,
											// String, Date, or Class
					mate = newPrimitiveWrapper(clazz, jsonObj.get("value"));
				} else if (clazz.equals(Object.class)
						&& jsonObj.containsKey("value")) {
					mate = jsonObj.get("value");
				} else { // Use the passed in class type
		...
		while (!done) {
			...
			case STATE_READ_START_OBJECT:
				...
				if (c == '{') {
					...
					throw new IOException(
							"Input is not valid JSON; does not start with '{' or '['");
				}
				break;
			...
			case STATE_READ_FIELD:
				...
				if (c == '"') {
					...
					throw new IOException("Expected quote at position "
							+ _in.getPos());
				}
				break;
			...
			case STATE_READ_VALUE:
				...
				if (c == '[') {
					Object o = readArray();
					if (field == null) { // field is null when you have an
											// untyped Object[], so we place
			...
			case STATE_READ_POST_VALUE:
				...
				if (c == '}' || c == -1) {
					...
					throw new IOException(
							"Object not ended with '}' at position "
									+ _in.getPos());
				}
				break;
			}
	...
	private Object readValue() throws IOException {
		...
		if (isDigit(c) || c == '-') {
			...
			throw new IOException("Unknown value type at position "
					+ _in.getPos());
		}
	}
	 ...
	 *         in JSON. These will be stored in simple forms like Double,
	 *         Boolean, null, Long, String, or JsonObject for non-primitive
	 *         types.
	 * @throws IOException
	 *             for stream errors or parsing errors.
	 */
	private Object readArray() throws IOException {
		...
		while (!stack.isEmpty()) {
			...
			while (true) {
				...
				} else if (c == '[') { // read a nested array
					stack.addFirst(array);
					JsonArray newArray = new JsonArray();
					array.add(newArray);
					stack.addFirst(newArray);
					break;
				} else { // read a value (any value but array)
					_in.unread(c);
					Object o = readValue();
					if (o != EMPTY_ARRAY) {
						array.add(o);
	...
	private String readToken(String token) throws IOException {
		...
		for (int i = 0; i < len; i++) {
			...
			if (loTokenChar != c) {
				throw new IOException("Expected token '" + token
						+ "' at position " + _in.getPos());
			}
		}
	...
	private Number readNumber(int c) throws IOException {
		...
		try {
			...
			throw new IOException("Too many digits in number at position "
					+ _in.getPos());
		}
		...
		while (!done) {
			...
			case STATE_STRING_SLASH:
				if (c == 'n') {
					...
					throw new IOException(
							"Invalid character escape sequence specified at position "
									+ _in.getPos());
				}
			...
			case STATE_HEX_DIGITS:
				if (c == 'a' || c == 'A' || c == 'b' || c == 'B' || c == 'c'
					...
					throw new IOException(
							"End of input reached before hex number read at position "
									+ _in.getPos());
				} else {
					throw new IOException(
							"Expected hexidecimal digits at position "
									+ _in.getPos());
				}
				break;
	...
	private Object[] tryOtherConstructors(Class c) throws IOException {
		...
		if (constructors.length == 0) {
			throw new IOException("Cannot instantiate '" + c.getName()
					+ "' - Primitive, interface, array[] or void at position "
					+ _in.getPos());
		}

		// Try each constructor (private, protected, or public) with default
		// values until
	...
	private Object newPrimitiveWrapper(Class c, Object rhs) throws IOException {
		if (c.equals(Byte.class) || c.equals(byte.class)) {
		...
		} else if (c.equals(Boolean.class) || c.equals(boolean.class)) { // Booleans
																			// are
																			// tokenized
																			// into
																			// Boolean.TRUE
			...
			try {
				...
				try { // If time zone is specified at the end in (EST), then
						// place this calendar
					...
					String tz = dateStr
							.substring(idx + 1, dateStr.length() - 1);
		...
		throw new IOException("Class '" + c.getName()
				+ "' requested for special instantiation.");
	}
	...
	private Class classForName(String name) throws IOException {
		if (name == null || name.length() == 0) {
			throw new IOException("Invalid class name specified, position: "
					+ _in.getPos());
		}
		try {
			...
			throw new IOException("Class instance '" + name
					+ "' could not be created at position " + _in.getPos());
		}
	}
	...
	private void patchUnresolvedReferences() throws IOException {
		...
		while (i.hasNext()) {
			...
			if (objReferenced.target == null) {
				System.err
						.println("Back referenced object does not exist,  @ref "
			...
			if (objToFix == null) {
				System.err
						.println("Referencing object is null, back reference, @ref "
								+ ref.refId + ", field '" + ref.field + '\'');
				continue;
			}
			...
			if (ref.index >= 0) { // Fix []'s and Collections containing a
				...
				if (field != null) {
					try {
						...
						throw new IOException(
								"Error setting field while resolving references '"
										+ field.getName() + "', @ref = "
										+ ref.refId);
					}
				}
		...
		while (i.hasNext()) {
			Map map = (Map) i.next();
			Collection<Map.Entry> entries = new ArrayList<Map.Entry>(map
					.entrySet());
			map.clear(); // reset Map
	...
	public int read(char[] cbuf, int off, int len) throws IOException {
		throw new IOException(
				"read(char[] _buf, int offset, int len) is not supported");
	}
	...
	private static class FastPushbackReader extends FilterReader {
		...
		public void unread(int c) throws IOException {
			if (_idx == 0) {
				throw new IOException(
						"unread(int c) called more than pushback buffer size ("
								+ _buf.length + "), position = " + _pos);
			}
			_pos--;
			_buf[--_idx] = c;
		}
