package game.russiablocks;

/**
 * File: ErsBlocksGame.java User: Administrator Date: Jan 15, 2003 Describe:
 * 俄罗斯方块的 Java 实现
 */
...
/**
 * 游戏主类，继承自JFrame类，负责游戏的全局控制。 内含 1, 一个GameCanvas画布类的实例引用， 2,
 * 一个保存当前活动块(ErsBlock)实例的引用， 3, 一个保存当前控制面板（ControlPanel）实例的引用;
 */
public class ErsBlocksGame extends JFrame {
	private static final long serialVersionUID = -3437372409676718223L;
	/**
	 * 每填满一行计多少分
	 */
	...
	private void createMenu(){
		...
		miSetBlockColor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae){
				Color newFrontColor = JColorChooser.showDialog(ErsBlocksGame.this, "Set color for block", canvas.getBlockColor());
		...
		miSetBackColor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae){
				Color newBackColor = JColorChooser.showDialog(ErsBlocksGame.this, "Set color for block", canvas.getBackgroundColor());
	...
	private class Game implements Runnable {
		public void run(){
			int col = (int)(Math.random() * (canvas.getCols() - 3)), style = ErsBlock.STYLES[(int)(Math.random() * 7)][(int)(Math.random() * 4)];
