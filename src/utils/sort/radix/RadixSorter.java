...
public class RadixSorter{
	...
	private final void array_radix_sort(int[] keys, int from, int len, int radix,
		...
		for(int i = 0; i < d; i++){
			...
			for(int k = 0; k < len; k++){
				...
				count[subkey]++;
				System.out.print("count[" + subkey + "]=" + count[subkey] + ",");
			}

			System.out.print("\n@ ");
			for(int j = 1; j < radix; j++){
				System.out.print("count[" + j + "]=" + (count[j] + count[j - 1]) + ", ");
	...
	private final void link_radix_sort(int[] keys, int from, int len, int radix,
	    int d){
	...
	private int link_radix_sort_collect(int[] keys, int from, int len, int radix,
