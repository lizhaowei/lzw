...
public class Stringutils{
	...
	private static String[] smsSplitByLength(String sSMS, final int LENGTH){
		...
		while(true){
			y = x + LENGTH;
			if (y > iContentLength) y = iContentLength;
			sTemp = sSMS.substring(x, y);
			l.add(sTemp);
			x = y;
			if (x == iContentLength) break;
		}
	...
	public static int hexToDec(String sn){
		if (null == sn && 0 == sn.trim().length()) throw new IllegalArgumentException("接收的参数 “" + sn + "” 不是十六进制表示形式！");

		final String P = "0x";
		if (sn.toLowerCase().startsWith(P)){
			final String regex = "[^\\da-fA-F]";
			int i = Regexutils.matcheRegexp(sn.substring(2), regex, false);
			if (0 != i) throw new IllegalArgumentException("接收的参数 “" + sn + "” 不是十六进制表示形式！");
			else{
	 ...
	 */
	public static String sbcTodbc(String str) throws UnsupportedEncodingException{
		if (null == str || 0 == str.trim().length()) return null;
		...
		for(int i = 0; i < c.length; i++){
			...
			if (c[i] > 65280 && c[i] < 65375) c[i] = (char)(c[i] - n);
		}
		return new String(c);
	}
	...
	public static String[] split(String str, final String regex){
		if (null == str || 0 == str.trim().length()) return null;
		if (null == regex || 0 == regex.trim().length()) return new String[]{str};
	 ...
	 * 
	 * @param st
	 * @param decodingCharset 使用指定字符集解码
	 * @return 异常，返回null；否则返回编码后的字符串
	 */
	public static String toGBKString(String st, final String decodingCharset){
		try{
			if (null == decodingCharset) return new String(st.getBytes(), GBK);
			else return new String(st.getBytes(decodingCharset), GBK);
